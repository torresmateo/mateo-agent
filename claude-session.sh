...
# Build session-specific variables
SESSION_VARS_MARKER="# === CLAUDE SESSION VARIABLES (auto-generated, do not edit) ==="
SESSION_VARS_END="# === END CLAUDE SESSION VARIABLES ==="

cat > /tmp/session-vars << 'SESSION_EOF'
...
if [ "$DB_ENABLED" = "true" ]; then
  echo "DATABASE_URL=postgresql://$DB_USER:$DB_PASSWORD@$DB_CONTAINER_NAME:5432/$DB_NAME" >> /tmp/session-vars
fi

# Add session metadata
echo "CLAUDE_SESSION_NAME=$SESSION_NAME" >> /tmp/session-vars
echo "CLAUDE_DB_ENABLED=$DB_ENABLED" >> /tmp/session-vars
echo "$SESSION_VARS_END" >> /tmp/session-vars
echo "" >> /tmp/session-vars

# Handle .env file
if [ -f .env ]; then
  # Remove existing session variables section if present
  if grep -q "$SESSION_VARS_MARKER" .env; then
    sed -i "/$SESSION_VARS_MARKER/,/$SESSION_VARS_END/d" .env
  fi

  # Prepend session variables to existing .env
  cat /tmp/session-vars .env > /tmp/.env.new
  mv /tmp/.env.new .env
else
  # Create new .env with session variables
  cat > .env << 'ENV_HEADER'
# Project environment variables
# This file is automatically added to .gitignore

ENV_HEADER
  cat /tmp/session-vars >> .env
  cat >> .env << 'ENV_FOOTER'
...
# injected at the top of .env by claude-session
#
# You can override them here or in .env.local files
