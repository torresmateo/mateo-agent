...
# Fixed credentials (no collision risk since each session has isolated container)
DB_USER="postgres"
...
cmd_start() {
  ...
  # Create setup script
  local setup_script="/tmp/setup-session-env-$container_name.sh"
...
# Create .env.claude-session with session-specific variables
cat > .env.claude-session << 'EOF'
# Session-specific environment variables
# Auto-generated by claude-session - DO NOT EDIT
# This file is automatically added to .gitignore
#
# Load this in your project's .env files with:
# source ../.env.claude-session (bash)
# or use dotenv to load it in your app

EOF

# Add DATABASE_URL if database is enabled
if [ "$DB_ENABLED" = "true" ]; then
  echo "DATABASE_URL=postgresql://$DB_USER:$DB_PASSWORD@$DB_CONTAINER_NAME:5432/$DB_NAME" >> .env.claude-session
fi

# Add session name for reference
echo "CLAUDE_SESSION_NAME=$SESSION_NAME" >> .env.claude-session
echo "CLAUDE_DB_ENABLED=$DB_ENABLED" >> .env.claude-session

chmod 600 .env.claude-session

# Create .env if not exists (for backwards compatibility)
if [ ! -f .env ]; then
  cat > .env << 'ENV_EOF'
# Project-specific secrets
# This file is automatically added to .gitignore
#
# Session-specific variables are in .env.claude-session
# You can source it here if needed:
...
# Copy to .env and fill in your values
#
# Session-specific variables (DATABASE_URL, etc.) are automatically
# provided in .env.claude-session - you can load it in your component's .env:
#
# In bash: source ../.env.claude-session
# In Node.js: require('dotenv').config({ path: '../.env.claude-session' })
EXAMPLE_EOF
fi
...
if [ -f .git/config ] || [ -f ../.git/config ]; then
  ...
  for pattern in .env .env.local .env.claude-session '.env.*.local' '*.key' '*.pem' '*credentials*.json' '*secrets*.json' '*.p12' '*.pfx'; do
...
SCRIPT_EOF
  ...
  docker exec \
    ...
    -e SESSION_NAME="$container_name" \
    "$container_name" /tmp/setup-env.sh 2>/dev/null || true
